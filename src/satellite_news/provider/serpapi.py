"""SerpApi Google News provider adapter."""
...
class SerpApiGoogleNewsProvider:
    ...
    def fetch_raw_articles(
        ...
        *,
        ...
        secret_env = api_key_env(provider, "SERPAPI_KEY")
        ...
        queries = query_templates(
            company=company,
            provider=provider,
            default_template='"{company_name}" satellite news',
        )
        if context.dry_run:
        ...
        endpoint = str(provider.options.get("endpoint") or DEFAULT_SERPAPI_ENDPOINT)
            ...
            try:
                ...
                for row in payload.get("news_results", ()) or ():
                    ...
                    article = build_raw_article(
                        ...
                        raw_text=str(row.get("snippet") or "") or None,
                        raw_payload=row,
                        metadata={
                            "serpapi_query": query,
